...
//LIBS += -LE:\opencv\build\x64\vc15\lib \
//-lopencv_world341d
...
void Menu::Menu_Geom()
{
    ...
    QAction *Act_geom_cutting = new QAction( tr("图像裁剪"), this);
    connect(Act_geom_cutting, SIGNAL(triggered()), I_MainWindow, SLOT(setCuttingTrue()));
    ...
    geom->addAction(Act_geom_cutting);
}
...
//    QAction *Act_enhance_candy = new QAction(tr("candy风格"));
//    connect(Act_enhance_candy, SIGNAL(triggered()), I_MainWindow, SLOT(candyImage()));

//    menu_cycleGan = new QMenu();
//    menu_cycleGan->addAction(Act_enhance_candy);
...
//    QAction *Act_enhance_CycleGan = new QAction(tr("风格迁移"), this);
//    Act_enhance_CycleGan->setMenu(menu_cycleGan);


	QMenu *enhance = addMenu(tr("图像增强"));
	enhance->addAction(Act_enhance_smooth);
    enhance->addAction(Act_enhance_sharpen);
//    enhance->addAction(Act_enhance_CycleGan);
}
...
//	connect(Act_help_info, SIGNAL(triggered()), this, SLOT(Help_info()));
    ...
    QAction *Act_cycleGan_candy = new QAction(tr("candy风格"), this);
    connect(Act_cycleGan_candy, SIGNAL(triggered()), I_MainWindow, SLOT(candyImage()));
    QAction *Act_cycleGan_feathers = new QAction(tr("feathers风格"), this);
    connect(Act_cycleGan_feathers, SIGNAL(triggered()), I_MainWindow, SLOT(feathersImage()));
    QAction *Act_cycleGan_la_muse = new QAction(tr("la_muse风格"), this);
    connect(Act_cycleGan_la_muse, SIGNAL(triggered()), I_MainWindow, SLOT(la_museImage()));

    QAction *Act_cycleGan_mosaic = new QAction(tr("mosaic风格"), this);
    connect(Act_cycleGan_mosaic, SIGNAL(triggered()), I_MainWindow, SLOT(mosaicImage()));
    QAction *Act_cycleGan_starry_night = new QAction(tr("starry_night风格"), this);
    connect(Act_cycleGan_starry_night, SIGNAL(triggered()), I_MainWindow, SLOT(starry_nightImage()));
    QAction *Act_cycleGan_the_scream = new QAction(tr("the_scream风格"), this);
    connect(Act_cycleGan_the_scream, SIGNAL(triggered()), I_MainWindow, SLOT(the_screamImage()));

    QAction *Act_cycleGan_udnie = new QAction(tr("udnie风格"), this);
    connect(Act_cycleGan_udnie, SIGNAL(triggered()), I_MainWindow, SLOT(udnieImage()));

    QMenu *CycleGan = addMenu(tr("风格迁移"));
    CycleGan->addAction(Act_cycleGan_candy);
    CycleGan->addAction(Act_cycleGan_feathers);
    CycleGan->addAction(Act_cycleGan_la_muse);
    CycleGan->addAction(Act_cycleGan_mosaic);
    CycleGan->addAction(Act_cycleGan_starry_night);
    CycleGan->addAction(Act_cycleGan_the_scream);
    CycleGan->addAction(Act_cycleGan_udnie);
}
...
//		}
//        QImage* img1 = img;
//        *img = img1->convertToFormat(QImage::Format_RGB888);
//    }
    if (src.empty()) {
        QMessageBox::information(this, tr("错误"), tr("打开图像失败!"));
    }
    ...
    QImage *img = new QImage();
    *img = imgchangeClass->cvMat2QImage(src);
...
{
	if (flag == true)
        menu->setStyleSheet("background-color: rgba(74, 79, 81, 1); color: rgb(187, 187, 187); ");
	else
