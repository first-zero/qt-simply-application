...
void ImageProcessing::init()
{
    ...
    cursor.movePosition(QTextCursor::End);		//设置光标位置
    outputEdit->setTextCursor(cursor);

    outputEdit->setWordWrapMode(QTextOption::NoWrap);				// 设置不换行
...
{
    // 标题大小
    setWindowTitle("图像处理系统");
    resize(static_cast<int> (QApplication::desktop()->width()*0.8),
           static_cast<int> (QApplication::desktop()->height()*0.8));
    move(static_cast<int> (QApplication::desktop()->width()*0.05),
         static_cast<int> (QApplication::desktop()->height()*0.01));
...
//    dock_Tool->setFixedWidth(100);
    dock_Tool->setFixedSize(QSize(QWIDGETSIZE_MAX,QWIDGETSIZE_MAX));
    dock_Tool->adjustSize();
    dock_Tool->setFixedSize(dock_Tool->size());
    ...
    dock_Color->setFixedWidth(450);
    dock_Prop->setMaximumHeight(240);
//    dock_Geom->setFixedSize(QSize(QWIDGETSIZE_MAX,QWIDGETSIZE_MAX));
//    dock_Geom->adjustSize();
//    dock_Geom->setFixedSize(dock_Tool->size());
//    dock_Gray->setFixedSize(QSize(QWIDGETSIZE_MAX,QWIDGETSIZE_MAX));
//    dock_Gray->adjustSize();
//    dock_Gray->setFixedSize(dock_Tool->size());
//    dock_Enhance->setFixedSize(QSize(QWIDGETSIZE_MAX,QWIDGETSIZE_MAX));
//    dock_Enhance->adjustSize();
//    dock_Enhance->setFixedSize(dock_Tool->size());
//    dock_Color->setFixedSize(QSize(QWIDGETSIZE_MAX,QWIDGETSIZE_MAX));
//    dock_Color->adjustSize();
//    dock_Color->setFixedSize(dock_Tool->size());
//    dock_Prop->setFixedSize(QSize(QWIDGETSIZE_MAX,QWIDGETSIZE_MAX));
//    dock_Prop->adjustSize();
//    dock_Prop->setFixedSize(dock_Tool->size());
...
//    splitDockWidget(dock_Prop, dock_Geom, Qt::Vertical);
...
void ImageProcessing::InitImage()
{
    ...
    imgLabel->setI_MainWindow(this);
    imgLabel->setScaledContents(true); // 图像填满标签
...
//// 图像数据比较
//bool ImageProcessing::matIsEqual(const cv::Mat mat1, const cv::Mat mat2) {
//    if (mat1.empty() && mat2.empty()) {
//        return true;
//    }
//    QImage a;
//    a.isNull();a.width(); a.size(); a.bits();a.bytesPerLine();
...
//    bool lvRet = memcmp(a.bits(), a.bits(), nrOfElements1) == 0;
//    return lvRet;
//}
...
{
//    if(Img == imgLabel->getImage())
//        return;
    imgLabel->setImage(Img);
//    imshow("showTest",imgChangeClass->QImage2cvMat(Img));
...
void ImageProcessing::WinPreview()
{
    ...
    QImage image = QImage(550, 550, QImage::Format_RGB888);
    image.fill(qRgb(255, 255, 255));
...
//    toolLayout->setRowStretch( 5, 1); // 第5行拉伸到最后

//    dock_Tool->setLayout(toolLayout);
...
void ImageProcessing::WinEnhance()
{
    ...
    slider_lineThreshold->setValue(80);
    ...
    enhanceWidget->setFixedWidth(520);
...
void ImageProcessing::undo()
{
    if (iter != imgVector[0].begin())			// 前面还有对象
    {
        iter--;
        //        imshow("test1", *iter);
//        Mat test = imgChangeClass->QImage2cvMat(*iter);
//                imshow("test2", test);
...
void ImageProcessing::autoSize()
{
    ...
    {
        // Qt:IgnoreAspectRatio 尺寸自由缩放
        // Qt:SmoothTransformation 边缘平滑处理
        Img = imgLabel->getImage().scaled((ImgscrollArea->width() - 2),
                                          (ImgscrollArea->width() - 2) / ImgRatio,
                                          Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    }
    else
    {
        Img = imgLabel->getImage().scaled((ImgscrollArea->height() - 2) * ImgRatio,
                                          (ImgscrollArea->height() - 2),
                                          Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    }
    //QImage Img = imgLabel->getImage().scaled(ImgscrollArea->width() - 2, ImgscrollArea->height() - 2, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
//    Mat temp = imgChangeClass->QImage2cvMat(Img);
    ShowImage(Img, true);
    output("图像大小：自适应窗口大小");
}
...
void ImageProcessing::rotateImageOK()
{
    ...
    if(line_degree->text().toInt() == 90 || line_degree->text().toInt() == -270)
        ...
        Img = geomClass->Rotate(imgLabel->getImage(), -line_degree->text().toInt());
...
// 图像裁剪
void ImageProcessing::setCuttingTrue() {
    imgLabel->setCuttingStatus(true);
    output("图像裁剪");
...
// candy风格迁移检测
void ImageProcessing::candyImage()
{
    QImage Img = enhanceClass->FastCycleGan(imgLabel->getImage(), "candy");
    ShowImage(Img, true);	// 图像保存
    output("风格迁移：candy风格迁移检测");
}

// feathers风格迁移检测
void ImageProcessing::feathersImage()
{
    QImage Img = enhanceClass->FastCycleGan(imgLabel->getImage(), "feathers");
    ShowImage(Img, true);	// 图像保存
    output("风格迁移：feathers风格迁移检测");
}

// la_muse风格迁移检测
void ImageProcessing::la_museImage()
{
    QImage Img = enhanceClass->FastCycleGan(imgLabel->getImage(), "la_muse");
    ShowImage(Img, true);	// 图像保存
    output("风格迁移：la_muse风格迁移检测");
}

// mosaic风格迁移检测
void ImageProcessing::mosaicImage()
{
    QImage Img = enhanceClass->FastCycleGan(imgLabel->getImage(), "mosaic");
    ShowImage(Img, true);	// 图像保存
    output("风格迁移：mosaic风格迁移检测");
}

// starry_night风格迁移检测
void ImageProcessing::starry_nightImage()
{
    QImage Img = enhanceClass->FastCycleGan(imgLabel->getImage(), "starry_night");
    ShowImage(Img, true);	// 图像保存
    output("风格迁移：starry_night风格迁移检测");
}

// the_scream风格迁移检测
void ImageProcessing::the_screamImage()
{
    QImage Img = enhanceClass->FastCycleGan(imgLabel->getImage(), "the_scream");
    ShowImage(Img, true);	// 图像保存
    output("风格迁移：the_scream风格迁移检测");
}

// udnie风格迁移检测
void ImageProcessing::udnieImage()
{
    QImage Img = enhanceClass->FastCycleGan(imgLabel->getImage(), "udnie");
    ShowImage(Img, true);	// 图像保存
    output("风格迁移：udnie风格迁移检测");
}
